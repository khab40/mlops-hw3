...
- Five ports were forwarded from the VM to the laptop during the assignment runs.
- The three required UIs were reachable through the forwarded ports: Grafana, Prometheus, and Langfuse.
- `.env` was created from `.env.example` and later populated with the local Langfuse keys plus serving configuration.
- BIRD data was loaded under `data/bird/`; the local copy contains the assignment SQLite databases and dev metadata.
...
- Grafana screenshots: `screenshots/grafana_before.png` and `screenshots/grafana_after.png`
...
The verify/revise loop did not add measurable execution accuracy on this eval set. The per-iteration pass rate is the evidence: attempt 1 was `10/30`, attempt 2 was `10/30`, and attempt 3 was `10/30`. The loop did provide observability value because Langfuse traces showed the full `generate_sql -> execute -> verify -> revise` waterfall and helped explain why multi-step requests were slow, but the revision policy did not recover wrong SQL. For the serving SLO, keeping the full graph available while using the fast `generate_sql -> execute` path under load was the right tradeoff.

What I would do with more time:

- Replace crude schema trimming with schema retrieval: rank tables/columns using question terms plus foreign-key neighborhoods, then include only the top schema slice.
...
- Build a small prompt-tuning set from the 20 consistently wrong eval questions, especially `formula_1`, `thrombosis_prediction`, and `toxicology`.

Final deliverables:

| File | Status |
|---|---|
| `REPORT.md` | Complete writeup with phase results and final summary |
